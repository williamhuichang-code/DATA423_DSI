  ...
  output$model_specific_config_ui <- renderUI({
    ...
    if (method == "glmnet" && mode == "specific") {
      ...
      controls <- c(controls, list(
        ...
        selectInput("svmRadialSigma_grid_mode", "Tuning grid", choices = c("Tune length default" = "default", "Custom sigma/C grid" = "custom"), selected = "default", width = "100%"),
        conditionalPanel(
          condition = "input.svmRadialSigma_grid_mode == 'custom'",
          sliderInput("svmRadialSigma_sigma_min", "Log10 sigma minimum", min = -4, max = 0, value = -3.2, step = 0.1, width = "100%"),
          sliderInput("svmRadialSigma_sigma_max", "Log10 sigma maximum", min = -4, max = 0, value = -2.7, step = 0.1, width = "100%"),
          sliderInput("svmRadialSigma_sigma_n", "Sigma values", min = 3, max = 20, value = 5, step = 1, width = "100%"),
          sliderInput("svmRadialSigma_c_min", "Log2 C minimum", min = -5, max = 8, value = -2, step = 1, width = "100%"),
          sliderInput("svmRadialSigma_c_max", "Log2 C maximum", min = -5, max = 8, value = 4, step = 1, width = "100%"),
  ...
  observeEvent(input$svmRadialSigma_Go, {
    ...
    tryCatch({
      timing <- system.time({
        ...
        if (specific_mode && grid_mode == "custom") {
          sigma_min <- min(input$svmRadialSigma_sigma_min %||% -2.5, input$svmRadialSigma_sigma_max %||% -1.3)
          sigma_max <- max(input$svmRadialSigma_sigma_min %||% -2.5, input$svmRadialSigma_sigma_max %||% -1.3)
          c_min <- min(input$svmRadialSigma_c_min %||% 0, input$svmRadialSigma_c_max %||% 5)
          c_max <- max(input$svmRadialSigma_c_min %||% 0, input$svmRadialSigma_c_max %||% 5)
          tune_grid <- expand.grid(
            sigma = signif(10^seq(sigma_min, sigma_max, length.out = input$svmRadialSigma_sigma_n %||% 7), 4),
            C = signif(2^seq(c_min, c_max, length.out = input$svmRadialSigma_c_n %||% 6), 4)
          )
          model <- caret::train(x = x, y = y_train, method = method,
                                metric = "RMSE", trControl = getTrControl(),
                                tuneGrid = tune_grid)
